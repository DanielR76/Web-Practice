...
function realizarPorcentaje() {
    do {
        ...
        for (let i = 0; i < porcentaje.length; i++) {
            porcentaje[i] = parseInt(prompt('Ingrese el porcentaje del corte ' + (i + 1), 0))
        }
...
function notasCorte() {
    porcentaje.forEach((element, index) => {
        let cantidadNotas = parseInt(prompt("Ingrese la cantidad de notas que serán evaluadas para el corte: " + (index + 1), 0))
    ...
    notasTotales.forEach((resultado1, indice1) => {
        ...
        porcentaje[indice1] = porcentaje[indice1] * (result / notasTotales[indice1].length)
        result = 0
    })
